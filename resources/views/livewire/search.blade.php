<div class="w-full">
    <input class="border border-slate-100 shadow-md p-2 w-full h-[50px] rounded focus:outline-none" wire:model="search"
        wire:keydown="researchUser" placeholder="Search...">
    @if ($search != '')
        @foreach ($users as $user)
            <a href="/profile/{{ '@' . $user->username }}">
                <div class="flex gap-2 my-2 items-center border rounded border-blue-300 p-3 shadow-blue-300 shadow-sm">
                    <img class="w-[50px] h-[50px] object-fill rounded-[50%] border border-blue-400"
