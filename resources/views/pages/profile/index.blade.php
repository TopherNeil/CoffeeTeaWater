<x-layout>
    ...
    <div class="w-[830px] h-auto flex flex-col rounded">
        <div
            class="mt-4 w-full flex flex-col items-center justify-center border border-slate-300 p-5 rounded-lg shadow-md bg-slate-200">
            ...
            <a href="{{ route('profile.edit') }}">
                <div class="group relative rounded-full h-[120px] w-[120px] flex justify-center items-center">
                    ...
                    <span
                        class="absolute inset-0 bg-black bg-opacity-50 flex justify-center items-center text-white text-sm font-semibold rounded-full opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                        Edit
                    </span>
                </div>
            </a>
        ...
        @foreach ($posts as $post)
            <div id="{{ $post->post_id }}"
                class="flex flex-col border border-slate-100 my-5 shadow-lg p-5 w-100 h-auto rounded-md relative">
                            ...
                            <div>
                                <x-options>
                                    <a class="rounded hover:bg-slate-200 hover:shadow-sm"
                                        href="/post/edit/{{ $post->post_id }}">{{ __('Edit') }}</a>
                ...
                <div class="mt-6 flex w-full gap-2">
                    ...
                    <a class="group" href="/post/{{ $post->post_id }}">
                        <x-button severity="custom" name="Comments({{ $post->comment_count }})"
                            active_state="text-orange-600 group-hover:text-orange-300"
                            inactive_state="text-gray-600 group-hover:text-orange-600" />
                    </a>
                </div>
            </div>
        @endforeach
