<nav
    {{ $attributes->merge(['class' => 'z-10 h-[100vh] w-[80px] py-5 bg-white flex flex-col justify-between items-center fixed left-0 shadow-md']) }}>
    <div role="button" class="w-[75px] h-[50px] flex items-center justify-center">
        <a href="/"
            class="text-xl hover:scale-125 transition ease-in-out delay-250 duration-300 font-bold ">{{ __('CTW') }}</a>
    </div>
    ...
    <x-dropdown>
        ...
        <form class="w-full" method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit"
                class=" w-full rounded hover:bg-slate-200 hover:shadow-sm">{{ __('Sign Out') }}</button>
        </form>
    </x-dropdown>
</nav>
